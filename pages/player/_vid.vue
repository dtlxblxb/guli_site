...
export default {
  ...
  mounted() {
    ...
    new Aliplayer({ // 前端直接向阿里云发请求进行视频播放
      id: 'J_prismPlayer',
      width: '100%',
      vid: this.vid,
  ...
  // // 页面渲染之前执行
  // created() {
  //   /* eslint-disable no-undef */ // 这里Aliplayer会报eslint错,添加注解可解决,代表忽略 no-undef 检测
  //   new Aliplayer({ // 这里没有用模块化的方式引Aliplayer即import方式(aliyun没提供export),只能按照传统方式引js和css,而created()在<template>前执行,所以任然会报Aliplayer is not defined错误
  //     id: 'J_prismPlayer',
  //     width: '100%'
